...
class BluecatApiTest < Test::Unit::TestCase
  def setup
    @connection = Bluecat.new('10.10.10.10', 'https', false, 'admin', 'admin')
  ...
  def test_rest_get
    endpoint = 'test'
    qs = 'qs'
    req = stub_request(:get, "%s://%s/api/1.0/%s/qs" %
  ...
  def test_rest_post
    endpoint = 'test'
    req = stub_request(:post, "%s://%s/api/1.0/%s/" %
      [@connection.scheme, @connection.host, endpoint]).to_return(:status => 200, :body => [{"test"=>'test'}].to_json)
  ...
  def test_rest_delete
    endpoint = 'test'
    id = 1
    req = stub_request(:delete, "%s://%s/api/1.0/%s/%s/" %
  ...
  def test_doql
    req = stub_request(:post, "%s://%s/services/data/v1.0/query/" %
  ...
  def test_get_next_ip
    ...
    req = stub_request(:get, "%s://%s/api/1.0/suggest_ip/?end_range=10.75.1.21&start_range=10.75.1.20&subnet_id=1" %
  ...
  def test_get_next_ip_empty
    ...
    req = stub_request(:get, "%s://%s/api/1.0/suggest_ip/?end_range=&start_range=&subnet_id=" %
      [@connection.scheme, @connection.host]).to_return(:status => 200, :body => @ip_suggest_output)
